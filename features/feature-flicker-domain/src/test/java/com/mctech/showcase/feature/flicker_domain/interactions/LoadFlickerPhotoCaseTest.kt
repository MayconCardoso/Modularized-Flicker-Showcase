...
class LoadFlickerPhotoCaseTest {
    private val tag             = "cat"
    ...
    private lateinit var useCase: LoadFlickerPhotoCase

    @Before
    fun `before each test`() {
        useCase = LoadFlickerPhotoCase(service, logger)
    }
    ...
    fun `should delegate calling`() = suspendedTestScenario(
        scenario = {
            whenever(service.loadFirstPageOfPhotos(any())).thenReturn(expectedValue)
        },
        action = {
            useCase.execute(tag)
        },
        assertions = {
            verify(service).loadFirstPageOfPhotos(tag)
        }
    )
    ...
    private fun failureAssertion(exception: Throwable, expectedException: Exception) = suspendedTestScenario(
        scenario = {
            whenever(service.loadFirstPageOfPhotos(any())).thenThrow(exception)
        },
        action = {
            useCase.execute(tag)
        },
